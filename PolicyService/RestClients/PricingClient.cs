...
public class PricingClient : IPricingClient
{
    ...
    public PricingClient(IConfiguration configuration, IDiscoveryClient discoveryClient)
    {
        ...
        var pricingServiceUri = configuration.GetValue<string>("PricingServiceUri")
            ?? throw new InvalidOperationException("PricingServiceUri configuration is missing.");
        var httpClient = new HttpClient(handler, false)
        {
            BaseAddress = new Uri(pricingServiceUri)
        };
